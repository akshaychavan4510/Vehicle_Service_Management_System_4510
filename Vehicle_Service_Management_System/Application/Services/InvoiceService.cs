...
namespace Vehicle_Service_Management_System.Application.Services
{
    public class InvoiceService
    {
        ...
        public async Task<InvoiceCreateViewModel> BuildCreateFormAsync(
            int bookingId)
        {
            ...
            if (booking.Status != BookingStatus.Completed)
            {
                throw new InvalidOperationException(
                    "An invoice can only be generated for a completed booking.");
            }

            if (booking.Invoice != null &&
                !booking.Invoice.IsDeleted)
            {
                throw new InvalidOperationException(
            ...
            InvoiceCreateViewModel model)
        {
            ...
            var submittedParts =
                model.SparePartsUsed?
                    .Where(p =>
                        ...
                        p.QuantityUsed > 0)
                    .ToList()
                ?? new List<InvoiceItemInputViewModel>();

            // --------------------------------------------------------
            // DUPLICATE PART VALIDATION
            // --------------------------------------------------------

            var duplicatePartIds =
                submittedParts
            ...
            {
                throw new InvalidOperationException(
                    "The same spare part was submitted more than once.");
            }
            ...
            {
                // ----------------------------------------------------
                // SERVER-SIDE LABOUR CALCULATION
                // ----------------------------------------------------
                ...
                {
                    if (submittedPart.QuantityUsed <= 0)
                    {
                        continue;
                    }

                    // IMPORTANT:
                    // Never trust UnitPrice or AvailableStock
                    ...
                    {
                        throw new KeyNotFoundException(
                            $"Spare part with ID {submittedPart.SparePartId} was not found.");
                    }
                    ...
                    {
                        throw new InvalidOperationException(
                            $"Invalid quantity for '{sparePart.PartName}'.");
                    }

                    if (submittedPart.QuantityUsed >
                        sparePart.StockQuantity)
                    {
                        throw new InvalidOperationException(
                            $"Not enough stock for '{sparePart.PartName}'. " +
                            $"Available: {sparePart.StockQuantity}, " +
                            $"requested: {submittedPart.QuantityUsed}.");
                    }
        ...
        // Used by PaymentController -> SelectInvoice
        // ============================================================
        public async Task<List<InvoiceListViewModel>>
            GetInvoicesWithOutstandingBalanceAsync()
        {
        ...
        public async Task<(
            IEnumerable<InvoiceListViewModel> Items,
            int TotalRecords)> GetPaginatedAsync(
                int page,
                int pageSize,
                string? search = null,
                ...
                DateTime? dateTo = null)
        {
            ...
            var viewModels =
                _mapper.Map<IEnumerable<InvoiceListViewModel>>(
                    items);
        ...
        public async Task<InvoiceDetailsViewModel?>
            GetByIdAsync(int id)
        {
            var entity =
                await _context.Invoices
            ...
            var model =
                _mapper.Map<InvoiceDetailsViewModel>(
                    entity);

            var subtotal =
                entity.LabourCharge +
                entity.SparePartsTotal;

            model.DiscountPercentage =
                subtotal > 0
                    ? (entity.Discount / subtotal) * 100
                    : 0;
        ...
        public async Task<InvoicePrintViewModel?>
            GetPrintAsync(int id)
        {
            var entity =
                await _context.Invoices
                    .Include(i => i.ServiceBooking)
            ...
            var model =
                _mapper.Map<InvoicePrintViewModel>(
                    entity);
            ...
            model.AmountInWords =
                NumberToWords(model.GrandTotal);
        ...
        public async Task<InvoiceUpdateViewModel?>
            GetForEditAsync(int id)
        {
            var invoice =
                await _context.Invoices
                    .FirstOrDefaultAsync(
            ...
            var subtotal =
                ...
                invoice.SparePartsTotal;

            var discountPercentage =
                subtotal > 0
                    ? (invoice.Discount / subtotal) * 100
                    : 0;
            ...
            InvoiceUpdateViewModel model)
        {
            ...
            if (model.DiscountPercentage.HasValue)
            {
                ...
                var newDiscount =
                    subtotal *
                    (model.DiscountPercentage.Value / 100m);
        ...
        public async Task<bool> MarkAsUnpaidAsync(
            int invoiceId)
        {
            var invoice =
                await _context.Invoices
        ...
        public async Task<bool> IsBookingInvoicedAsync(
            int bookingId)
        {
            return await _context.Invoices
                .AnyAsync(
        ...
        public async Task<InvoiceStatisticsViewModel>
            GetStatisticsAsync(
                DateTime? dateFrom = null,
                DateTime? dateTo = null)
        {
            ...
            var stats =
                new InvoiceStatisticsViewModel
                {
                    TotalInvoices =
                        invoices.Count,
